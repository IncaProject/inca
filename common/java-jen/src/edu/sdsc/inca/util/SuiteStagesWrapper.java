...
public class SuiteStagesWrapper extends XmlWrapper {
  ...
  public void save() throws CrypterException, IOException {
    ...
    String xml = this.suiteStages.xmlText( xmloptions );
  ...
  public Vector<SeriesConfig> seriesConfigCrossProduct(
    ...
    Vector<String> seriesConfigCPText = this.resources.expand(
      seriesConfig.toString(), resource, null
    );
    Vector<SeriesConfig> seriesConfigCP = new Vector<SeriesConfig>();
    for ( String seriesConfigTxt : seriesConfigCPText ) {
      try {
        seriesConfigCP.add( SeriesConfig.Factory.parse(seriesConfigTxt) );
  ...
  public static void main(String[] args) {
    if ( args.length != 1 && args.length != 2) {
      ...
      System.err.println("Usage: SuiteStagesWrapper <inca file> [<dir>]");
      System.exit( 1 );
    }
    String incaFile = args[0];
    String expandedDir = null;
    if ( args.length == 2 ) expandedDir = args[1];
    logger.info
