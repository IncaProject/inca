...
public class SuiteStagesWrapperTest extends TestCase {
  ...
  public void testModify() throws Exception {
    ...
    String suiteXmlOrig = sampleSuite.getSuiteDocument().getSuite().xmlText
      ( xmlOptions );
    String suiteXml = suiteStagesWrapper.getSuiteDocument().getSuite().xmlText
      ( xmlOptions );
    suiteXmlOrig = suiteXmlOrig.replaceAll( "<xml-fragment.*>", "" );
    suiteXml = suiteXml.replaceAll( "<xml-fragment.*>", "" );
    suiteXmlOrig = suiteXml.replaceAll( "<version>?=", "<version>" );
  ...
  public void testSetResourcesWrapper() throws Exception {
    ...
    SuiteWrapper suiteChange = SuiteWrapperTest.createSampleSuite(repositories);
    suiteStagesWrapper.modify( suiteChange );
    XmlOptions opt = XmlWrapper.getPrettyPrintOptions();
    suiteStagesWrapper.getDocument().save(File.createTempFile("samplesuite", "xml"),opt);
    ...
    suiteStagesWrapper.getDocument().save( File.createTempFile("samplesuitechanges", "xml"), opt );
    ...
    assertEquals( "gcc not modified", gcc.xmlText(opt), gccnew.xmlText(opt) );
