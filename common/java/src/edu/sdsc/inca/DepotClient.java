...
public class DepotClient extends Client {
  ...
  public void insertReport(String resource,
    ...
    StringBuffer data = new StringBuffer(resource);
   ...
   */
  public String queryInstance(String nickname, String resource, String target, Date collected)
    throws IOException, ProtocolException, XmlException {

	String milliSeconds = String.valueOf(collected.getTime());
    String[] xml =
      queryDialog(Protocol.QUERY_INSTANCE_COMMAND, nickname + "\n" + resource + "\n" + target + "\n" + milliSeconds);
   ...
   * @throws XmlException if the Depot response cannot be parsed
   */
  public String queryInstanceById(long instanceId, long configId)
    throws IOException, ProtocolException, XmlException {

    String[] xml =
      queryDialog(Protocol.QUERY_INSTANCE_BY_ID_COMMAND, instanceId + " " + configId);
    if ( xml != null && xml.length > 0 ) {
      return xml[0];
    } else {
      return null;
  ...
  public String[] queryPeriod(Date begin, Date end, String expr)
    ...
    return queryDialogWithNoSoTimeout(Protocol.QUERY_PERIOD_COMMAND, request);
  }
   ...
   */
  public void insertKbArticle(String articleXml) throws IOException, ProtocolException
  {
    dialog(Protocol.KBARTICLE_INSERT_COMMAND, articleXml);
  }
   ...
   * @throws ProtocolException
   */
  public void deleteKbArticle(String articleId) throws IOException, ProtocolException
  {
    dialog(Protocol.KBARTICLE_DELETE_COMMAND, articleId);
  }

  /**
   * Asks the Depot for a list of its peer Depots
   *
   * @return peer depot URIs, null if none
   * @throws IOException
   * @throws ProtocolException
   */
  public String[] queryDepotPeers() throws IOException, ProtocolException
  {
    String result = dialog(Protocol.QUERY_DEPOT_PEERS_COMMAND, "");
    String[] uris = result.split("\\n");

    if (uris.length == 1 && uris[0].length() < 1)
      return null;
   ...
   * @throws ProtocolException
   */
  public String queryReporterSeries() throws IOException, ProtocolException
  {
    return dialog(Protocol.QUERY_REPORTERS_COMMAND, "");
  }
   ...
   * @throws ProtocolException
   */
  public String queryReporterSeriesDetail() throws IOException, ProtocolException
  {
    return dialog(Protocol.QUERY_REPORTERS_DETAIL_COMMAND, "");
  }
