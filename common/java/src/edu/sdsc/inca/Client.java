...
public class Client extends Component {
  ...
  /**
   *
   * @param dn the DN of the entity whose permission will be revoked
   * @param action the action for which permission will be revoked
   * @throws IOException on read/write error
   * @throws ProtocolException on an invalid message
   */
  public void commandRevoke(String dn, String action) throws IOException, ProtocolException {
    char quote = dn.indexOf('"') < 0 ? '"' : '\'';
    this.dialog(Protocol.REVOKE_COMMAND, quote + dn + quote + " " + action);
  }

  /**
   *
   * @param action the action for which all permissions will be revoked
   * @throws IOException on read/write error
   * @throws ProtocolException on an invalid message
   */
  public void commandRevokeAll(String action) throws IOException, ProtocolException {
    this.dialog(Protocol.REVOKE_ALL_COMMAND, action);
  }
